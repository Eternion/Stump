...
namespace Stump.Server.WorldServer.Game.Fights.Buffs.Customs
{
    public class DisableStateBuff : Buff
    {
        ...
        {
            if (Delay == 0)
                return new FightTemporaryBoostStateEffect(Id, Target.Id, Duration, (sbyte)Dispellable, (short)Spell.Id, EffectFix?.ClientEffectId ?? Effect.Id, 0, -100, (short)StateBuff.State.Id);
