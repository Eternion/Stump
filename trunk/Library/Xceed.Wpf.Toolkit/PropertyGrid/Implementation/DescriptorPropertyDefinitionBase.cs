    ...
    public FrameworkElement GenerateEditorElement( PropertyItem propertyItem )
    {
      ...
      if( _propertyParent.EditorDefinitions != null )
      {
        ...
        if( editorElement == null )
          editorElement = CreateCustomEditor( propertyItem, _propertyParent.EditorDefinitions[ GetPropertyName() ] );

        // Priority #3: Custom Editors based on the type (same for all PropertyDescriptors).
        if( editorElement == null )
          editorElement = CreateCustomEditor(  propertyItem, _propertyParent.EditorDefinitions[ GetPropertyType() ] );
      }
