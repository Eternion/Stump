...
namespace Stump.Tools.Proxy
{
    public sealed class HandlerManager
    {
        ...
        ///   Value : Target method
        /// </summary>
        private readonly Dictionary<Type, Tuple<Handler, Delegate>> m_handlers =
            new Dictionary<Type, Tuple<Handler, Delegate>>();
        ...
        public void Register(Type type)
        {
            ...
            foreach (MethodInfo method in methods)
            {
                ...
                try
                {
                    ...
                    {
                        RegisterHandler(((Handler)attribute).Type, (Handler)attribute, handlerDelegate);
                    }
                }
                catch (Exception e)
                {
                    ...
                    throw new Exception("Unable to register PacketHandler " + handlerStr +
                                        ".\n Make sure its arguments are: " + typeof(Message).FullName + ", " +
                                         typeof(DerivedConnexion).FullName +
                                        ".\n" + e.Message);
                }
            }
        }

        internal void RegisterHandler(Type msgType, Handler handler, Delegate target)
        {
            ...
            m_handlers.Add(msgType,
                           target != null
                               ? new Tuple<Handler, Delegate>(handler, target)
                               : new Tuple<Handler, Delegate>(handler, null));
        }


        internal bool Dispatch(Message message, DerivedConnexion sender)
        {
            Tuple<Handler, Delegate> handler;
            if (m_handlers.TryGetValue(message.GetType(), out handler))
            {
                ...
                handler.Item2.DynamicInvoke(message, sender);
                return true;
            }
            return false;
        }
